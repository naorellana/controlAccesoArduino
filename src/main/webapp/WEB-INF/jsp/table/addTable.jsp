...
<p> <!-- Se recorre todo el array porque no se puede especificar una posicion del vector-->
  value="${registro[0].campo}"
  value="${registro[0].tipoVar}"
  value="${registro[0].checkPrimary}"
  value="${registro[0].checkNull}"
  value="${registro[0].checkUnique}"

    ${registro[0].campo}
    ${registro[1].campo}
    <c:forEach var="temp" items="${registro}">   
        <c:out value="${temp.campo}"></c:out>
    </c:forEach></p>
